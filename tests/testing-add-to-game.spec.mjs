...
function savedLevel(mapId = 'library_level', mapName = 'Library Level') {
  return {
    version: 1,
    levels: [{
...
test('Add to Game creates an independent game workspace scene and stages world registration', async ({ page }) => {
  const originalLibrary = savedLevel();
  ...
  const dialog = page.getByRole('dialog', { name: 'Add Testing Level to Game' });
...
test('Add to Game refuses to overwrite a game scene with the same ID', async ({ page }) => {
  await page.addInitScript(({ key, value }) => {
    localStorage.setItem(key, JSON.stringify(value));
  }, { key: TESTING_LIBRARY_KEY, value: savedLevel('scene_lab', 'Collision Level') });
